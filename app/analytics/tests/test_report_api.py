...
@pytest.mark.parametrize(
    ...
    (
        (
            settings.BASE_DIR + "/analytics/tests/test_data/test_removed.xlsx",
            status.HTTP_201_CREATED,
        ),
        (
            settings.BASE_DIR + "/analytics/tests/test_data/test_added.xlsx",
            status.HTTP_201_CREATED,
        ),
        (
            settings.BASE_DIR + "/analytics/tests/test_data/test_empty.xlsx",
            status.HTTP_201_CREATED,
        ),
        (
            settings.BASE_DIR + "/analytics/tests/test_data/test_bad.txt",
...
def test_report_list_retrieving(api_client, report_factory):
    ...
    assert all(
        [
            field in data[0]
            for field in ("id", "uploaded", "processing_finished", "status", "result")
        ]
    )
...
def test_non_exists_report_handling(api_client, report_factory):
    ...
    response = api_client.get(
        reverse("report-detail", kwargs={"pk": report.id + 10000})
    )
